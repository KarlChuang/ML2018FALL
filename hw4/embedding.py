...
TRAIN_Y_VECTOR_SIMPLE_PATH = path.join('data', 'train_y_simple_vector.pt')
TEST_X_FILE_PATH = path.join('data', 'test_x.csv')
SEGMENTATION_FILE_PATH = path.join('data', 'segmentation.txt')
SIMPLE_SEGMENTATION_FILE_PATH = path.join('data', 'simple_segmentation.txt')
EMBEDDING_FILE_PATH = path.join('model', 'embedding' + str(EMBEDDING_DIMENSION) + '.model')
SIMPLE_EMBEDDING_FILE_PATH = path.join('model', 'simple_embedding' + str(EMBEDDING_DIMENSION) + '.model')

def get_train_x_vector_path(num):
    """
    use for cutting train_x file
    """
    return path.join('data', 'train_x_vector_simple', 'part%s.pt' % (int(num)))

def get_test_x_vector_path(num):
    """
    use for cutting test_x file
    """
    return path.join('data', 'test_x_vector_simple', 'part%s.pt' % (int(num)))
...
def simple_segmentation(input_file_path, output_file_path):
    """
    cut the sentences to words
    """
    with open(output_file_path, 'w', encoding='utf-8') as output_file:
...
def transfer_train_x_to_vector(embedding_model, train_x_file_path, train_y_file_path, batch_size):
    ...
    with open(train_x_file_path, 'r', encoding='utf-8') as file:
        with open(train_y_file_path, 'r') as file_y:
            ...
            torch.save(y_list, TRAIN_Y_VECTOR_SIMPLE_PATH)
            print()
...
def transfer_test_to_vector2(embedding_model, test_x_file_path):
    """
    cut testing data to small data and save as vector
    """
    with open(test_x_file_path, 'r', encoding='utf-8') as file:
        ...
        for idx, line in enumerate(file):
            ...
            temp_x_list.append(vector_list)
        if len(temp_x_list) > 0:
            torch.save(temp_x_list, './data/a_vector2.csv')
        print()


if __name__ == "__main__":
    # simple_segmentation(TRAIN_X_FILE_PATH, SIMPLE_SEGMENTATION_FILE_PATH)
    # embedding(SEGMENTATION_FILE_PATH, EMBEDDING_FILE_PATH)
    # embedding(SIMPLE_SEGMENTATION_FILE_PATH, SIMPLE_EMBEDDING_FILE_PATH)
    EMBEDDING_MODEL = word2vec.Word2Vec.load(EMBEDDING_FILE_PATH)
    # EMBEDDING_MODEL = word2vec.Word2Vec.load(SIMPLE_EMBEDDING_FILE_PATH)
    # transfer_train_x_to_vector(EMBEDDING_MODEL, TRAIN_X_FILE_PATH,
    #                            TRAIN_Y_FILE_PATH, 10000)
    # transfer_test_to_vector(EMBEDDING_MODEL, TEST_X_FILE_PATH, 10000)
    transfer_test_to_vector2(EMBEDDING_MODEL, './data/a.csv')
